...
public class RegulatorConection extends Thread {
	...
	byte[][] listener_response() throws IOException {// int[] tl
		...
		if ((desired && (length == 7)) == false) {
			byte[][] message_response = new byte[9][11];// o 9 é un exemplo,
														// teño que poñer o
														// axeitado
			boolean desired1 = (message1[0] == 0x02) && (message1[3] == 2);
			while (desired1 == false) {
				return null;
			}
			// if (desired1==true){
			// this.waitingResponse=false; //tengo que incluir los errores de
			// ack
			int length_mes;
			int id_m;
			int val = 0;
			int error = 0;
			/**
int val_lent0=(int)message[7 + 2]&0xff;
int val_len1=(int)message[ 2]&0xff;
			if (desired) {
				length = length - 7;
				if ( val_lent0< 0)
					val = 256 + val_lent0;
				...
				if (message[2] < 0)
					...
					val = (int) message[1];
				length_mes = val * 256 + val_len1;
				id_m = message[3];
			}
		
			if ((length - 4) != length_mes)
				error = 1;
			else if (id_m != 2)
				error = 2;**/
			...
			if (error == 0) {
				...
				if (message1[13] != 0) {
					while (a < (message1.length)) {
						a = a + next_pos;// para que mostre todolos grupos esto ten que vir aqui
						if (message1[4 + 9 + next_pos] != 0) {
							typeTemp e = new typeTemp();
							message_response[i][0] = message1[4 + 8 + next_pos];
							System.out.println("grupo "
									+ message[4 + 8 + next_pos]);
							message_response[i][1] = message1[4 + 10 + next_pos];
							System.out.println("color "
									+ message[4 + 10 + next_pos]);
							message_response[i][2] = message1[4 + 11 + next_pos];// System.out.println("veo2 "+message[4+11+
																					// next_pos]);
							message_response[i][3] = message1[4 + 12 + next_pos];// System.out.println("veo2 "+message[4+12+
																					// next_pos]);
							message_response[i][4] = message1[4 + 13 + next_pos];// System.out.println("voeo-1? "+message[4+13+
																					// next_pos]);
							...
							if (message1[4 + 9 + next_pos] > 1) {
								...
								e.Timer_last2 = ((b2[1] & 0xff) * 256) + b2[0]
										& 0xff;
							...
							while (io < this.List_ID.size()
									&& contains == false) {
								if (this.List_ID.get(io) == e.ID)
									contains = true;
								io++;
							}

							if (contains == false) {
								System.out.println("entra no contains");
								this.List_temp.add(e);
								this.List_ID.add(e.ID);
							} else {
								num = io - 1;
						...
						next_pos = ((int) message1[9 + 4 + old_pos] * 7) + 2
								+ old_pos;
						old_pos = next_pos;
						i++;
			...
			for (int j = 0; j < this.List_temp.size(); j++) {
				System.out.println("tempos na lista " + this.List_ID.get(j)
						+ " " + this.List_temp.get(j).Timer_last + "color: "
						+ this.List_temp.get(j).color);
			}
	...
	public void run() {
		...
		while ((this.isRunning == true)) {
			...
			try {
				if (requestSocket == null) {
					System.out.println("antes do while(!connect)");
					requestSocket = new Socket(Activator.spatRegIp, 21000);
					newConnection = true;
				} else if (requestSocket.isClosed()) {	System.out.println("antes do while(!connect)");
					requestSocket = new Socket(Activator.spatRegIp, 21000);
					newConnection = true;
				} else
					newConnection = false;
			...
			if (newConnection == true) {
				byte[] msg = createPetitionTlTimes(intersectionId, arrayTLtopo);
				newConnection = false;// porque 0x55
				sendMessage(msg);
				System.out.println("msg sent");
				this.waitingACK = true;
				// this.waitingResponse=true;
			}
			while (this.go && this.isRunning) {
				message_end = null;System.out.println("entro no go");
				try {
	...
	public byte[] createPetitionTlTimes(int idreg, int[] tlListInt) {
		...
		for (int i = 0; i < tlListInt1.length; i++) {
			...
			System.out.print("grupo semafórico " + tlListInt1[i] + " " + i + 6);
		}
		System.out.println("fin peticion " + (sizeMessage - 1));
		/******************************/
